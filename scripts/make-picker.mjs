...
// Клик по фото — увеличить; галочка в углу — выбрать; в просмотре S/пробел — выбрать, ←/→ листать.
// Использование: node scripts/make-picker.mjs <папка|путь>   (old, 2024, либо любой путь)
...
const prefix = insideLib ? rel : dir;                 // префикс путей в экспорте (src-совместимый, если внутри фототеки)
const label = path.basename(dir.replace(/[/\\]+$/, "")) || "photos";

fs.mkdirSync("pickers", { recursive: true });
const files = fs.readdirSync(dir).filter((f) => /\.jpe?g$/i.test(f)).sort();
console.log(`${label} (${dir}): ${files.length} фото, генерирую миниатюры…`);
