#!/usr/bin/env node
// Ингест фото из inbox/ в каталог сайта: EXIF-дата, сжатие до 3840px, YAML-заготовка.
// Темы потом проставляет Claude Code. Оригиналы держи в своей фототеке.
...
if (files.length === 0) {
  console.log("inbox пуст — кинь фото в ./inbox и запусти снова: npm run ingest");
  process.exit(0);
}
...
if (added.length) console.log(`\nДальше: попроси Claude «каталогизируй новые фото», затем npm run lqip и npm run build`);
